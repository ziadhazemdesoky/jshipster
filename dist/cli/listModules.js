...
export async function listModules(searchTerm) {
    ...
    const npmModules = await fetchNpmModules('jshipster-', searchTerm);
...
async function fetchNpmModules(prefix, searchTerm) {
    ...
    try {
        ...
        const packages = data.objects.map((obj) => obj.package.name);
        const userModules = packages.filter((mod) => mod.startsWith(prefix));
        if (searchTerm) {
            return userModules.filter((mod) => mod.includes(searchTerm));
        }
        return userModules;
    }
    catch (error) {
